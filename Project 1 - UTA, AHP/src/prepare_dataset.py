...
selects specific indicators as criteria, and adds distance from Poznań.
"""
...
CAPITALS_FILE = PROJECT_ROOT / "data" / "european_capitals.json"
OUTPUT_FILE = PROJECT_ROOT / "data" / "dataset.csv"
...
def find_raw_csv() -> pathlib.Path:
    ...
    if not csvs:
        ...
        print(f"and place the CSV file in: {RAW_DATA_DIR}")
        sys.exit(1)
    if len(csvs) > 1:
